"""Local pipeline for the from-scratch 125M SLM build (Phases 0 to 4).

Runs entirely on this machine: no cloud account, no cloud image, no cloud
volume. Parallelism across shards uses a local multiprocessing pool sized to
your CPU count (override with --workers on any phase that supports it).
"""
...
def _ensure_dirs() -> None:
    for d in (config.CLEAN_DIR, config.CORPUS_DIR, config.TOKENIZER_DIR,
              config.TRAIN_TOKENS_DIR, config.VAL_TOKENS_DIR):
...
# ---- Phase 0: smoke test + measure (cost: $0, a few minutes of your time) ----
...
# ---- Phase 1: stream + clean, one worker (local process) per parquet shard ----
...
def clean_shard(source_name: str, url: str, shard_index: int, token_cap: int) -> dict:
    ...
    out_dir = os.path.join(config.CLEAN_DIR, source_name)
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"shard-{shard_index:03d}.txt")
...
def _clean_shard_star(args):
    return clean_shard(*args)
...
def save_report(report: dict) -> None:
    with open(os.path.join(config.CLEAN_DIR, "phase1_report.json"), "w", encoding="utf-8") as fh:
        json.dump(report, fh, indent=2)


def phase_clean(fineweb_shards: int = 1, only: str = "", workers: int | None = None) -> None:
    _ensure_dirs()
    ...
    n_workers = workers or min(len(work), os.cpu_count() or 4)
    print(f"Launching {len(work)} clean workers across {n_workers} local processes...")
    with mp.Pool(n_workers) as pool:
        results = pool.map(_clean_shard_star, work)
...
DECONTAM_NGRAM = 13
SIG_DIR = os.path.join(config.DATA_ROOT, "tmp", "minhash_sigs")
NEAR_DUPS_PATH = os.path.join(config.DATA_ROOT, "tmp", "near_dups.json")
...
def minhash_shard(shard_basename: str) -> dict:
    ...
    path = os.path.join(config.CLEAN_DIR, "case-law", shard_basename)
    sigs, idxs = [], []
    ...
    np.savez(os.path.join(SIG_DIR, f"{shard_basename}.npz"),
...
def build_near_dups() -> int:
    ...
    for npz_path in sorted(glob.glob(os.path.join(SIG_DIR, "*.npz"))):
...
def write_corpus_shard(source_name: str, shard_basename: str) -> dict:
    ...
    in_path = os.path.join(config.CLEAN_DIR, source_name, shard_basename)
    out_dir = os.path.join(config.CORPUS_DIR, source_name)
    os.makedirs(out_dir, exist_ok=True)
    seen: set[str] = set()
    kept = clean_chars = 0
    reasons = {"near_dup": 0, "exact_dup": 0, "contaminated": 0, "kept": 0}
    with open(in_path, encoding="utf-8") as fin, \
            open(os.path.join(out_dir, shard_basename), "w", encoding="utf-8") as fout:
...
def _write_corpus_shard_star(args):
    return write_corpus_shard(*args)
...
def write_phase2_report(results: list) -> dict:
    ...
    with open(os.path.join(config.CORPUS_DIR, "phase2_report.json"), "w", encoding="utf-8") as fh:
        json.dump(report, fh, indent=2)
    return report


def phase_dedup(compute_sigs: bool = True, workers: int | None = None) -> None:
    ...
    if compute_sigs:
        ...
        with mp.Pool(min(n_workers, len(names))) as pool:
            pool.map(minhash_shard, names)
    ...
    with mp.Pool(min(n_workers, len(work))) as pool:
        results = pool.map(_write_corpus_shard_star, work)
...
def _corpus_line_iter():
    for path in sorted(glob.glob(os.path.join(config.CORPUS_DIR, "*", "*.txt"))):
        with open(path, encoding="utf-8") as fh:
            for line in fh:
                line = line.rstrip("\n")
                if line:
...
def tokenize_shard(source_name: str, shard_index: int, num_shards: int) -> dict:
    ...
    train_path = os.path.join(config.TRAIN_TOKENS_DIR, f"{source_name}-{shard_index:03d}.bin")
    val_path = os.path.join(config.VAL_TOKENS_DIR, f"{source_name}-{shard_index:03d}.bin")
    buf: list[int] = []
    win_count = n_train = n_val = 0
    corpus_files = sorted(glob.glob(os.path.join(config.CORPUS_DIR, source_name, "*.txt")))
...
def _tokenize_shard_star(args):
    return tokenize_shard(*args)
...
def write_token_index(results: list) -> dict:
    ...
    with open(os.path.join(config.TOKENS_DIR, "index.json"), "w", encoding="utf-8") as fh:
...
def phase_tokenize(workers: int | None = None) -> None:
    work = [(name, i, n) for name, n in TOKENIZE_SHARDS.items() for i in range(n)]
    n_workers = workers or min(len(work), os.cpu_count() or 4)
    print(f"Launching {len(work)} tokenize workers across {n_workers} local processes...")
    with mp.Pool(n_workers) as pool:
        results = pool.map(_tokenize_shard_star, work)
    write_token_index(results)
...
def ocr_sample(n_docs: int = 3000) -> dict:
    ...
    with open(config.CLEAN.dict_path, encoding="utf-8", errors="ignore") as fh:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Local SLM-125M data pipeline (Phases 0-4)")
    ...
    p.add_argument("--n-per-source", type=int, default=2000)

    p = sub.add_parser("clean")
    p.add_argument("--fineweb-shards", type=int, default=1)
    p.add_argument("--only", type=str, default="")
    p.add_argument("--workers", type=int, default=None)

    p = sub.add_parser("dedup")
    p.add_argument("--no-compute-sigs", dest="compute_sigs", action="store_false")
    p.add_argument("--workers", type=int, default=None)
    ...
    if args.phase == "smoke-test":
        smoke_test(args.n_per_source)
    elif args.phase == "measure":
        measure_sources(args.n_per_source)
    elif args.phase == "clean":
        phase_clean(args.fineweb_shards, args.only, args.workers)
    elif args.phase == "dedup":
        phase_dedup(args.compute_sigs, args.workers)
    elif args.phase == "tokenizer":
        train_tokenizer()
    elif args.phase == "tokenize":
        phase_tokenize(args.workers)
    elif args.phase == "ocr":
        ocr_sample(args.n_docs)
