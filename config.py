"""Single source of truth for the from-scratch 125M SLM build (local machine version)."""
...
# Project identity / paths (all paths are local — no cloud volume).
PROJECT = "slm-125m"
HF_REPO = "your-user/slm-125m-base"  # set to your own HF namespace for Phase 6 (not in this brief)

# Override with an env var if you want the corpus on a different disk,
# e.g. SLM_DATA_ROOT=/mnt/bigdisk/slm-125m-data
DATA_ROOT = os.environ.get("SLM_DATA_ROOT", os.path.join(os.getcwd(), "data"))
CLEAN_DIR = os.path.join(DATA_ROOT, "clean")          # Phase 1
CORPUS_DIR = os.path.join(DATA_ROOT, "corpus")        # Phase 2
TOKENIZER_DIR = os.path.join(DATA_ROOT, "tokenizer")  # Phase 3
TOKENS_DIR = os.path.join(DATA_ROOT, "tokens")        # Phase 4
TRAIN_TOKENS_DIR = os.path.join(TOKENS_DIR, "train")
VAL_TOKENS_DIR = os.path.join(TOKENS_DIR, "val")
CKPT_DIR = os.path.join(DATA_ROOT, "checkpoints")     # Phase 5 (not in this brief)
BASE_CKPT_DIR = os.path.join(CKPT_DIR, "base")
RESUME_CKPT_PATH = os.path.join(CKPT_DIR, "ckpt.pt")
...
class CleanConfig:
    ...
    # Native Windows has no /usr/share/dict/words; point at a local wordlist
    # shipped next to this file (SCOWL-style, one alphabetic word per line).
    dict_path: str = os.path.join(os.path.dirname(os.path.abspath(__file__)), "words_alpha.txt")
